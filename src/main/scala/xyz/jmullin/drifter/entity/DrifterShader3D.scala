...
object DrifterShader3D {
  ...
  abstract class Setter(val uniformName: String, global: Boolean) extends SetterGDX {
    fun isGlobal(shader: BaseShader, inputID: Int) = global
  ...
  fun setter[T](uniformName: String, f: (BaseShader, Renderable, Attributes) -> T, global: Boolean) = new Setter(uniformName, global) {
    override fun set(shader: BaseShader, inputID: Int, renderable: Renderable, attributes: Attributes)f(shader, renderable, attributes) match {
      case a: Float -> shader.set(inputID, a)
      case a: Int -> shader.set(inputID, a)
      case a: Vector2 -> shader.set(inputID, a)
  ...
  fun setter[T](uniformName: String, v: T, global: Boolean): Setter = setter[T](uniformName, (s: BaseShader, r: Renderable, a: Attributes) -> v, global)
}